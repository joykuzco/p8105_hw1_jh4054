...
The mean function will only work for some of the variables. The mean function works for the numeric variable since the inputs are already numeric. The mean function works for the logical vector since the outputs "TRUE" and "FALSE" are coded as numeric values 0 and 1, respectively. The character vector "vec\_char" and factor vector "vec\_fact" have categorical variables which cannot be averaged numerically.
...
We can convert the character "vec\_char" and factor variables "vec\_factor" into numeric variables using the "as.numeric" function
